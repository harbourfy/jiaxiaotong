        ...
        if (baseModel.getCode() == null){
            if(baseModel.getStatus().equalsIgnoreCase(ResultCode.SUCCESS)){
                if (baseModel.getActionType().equalsIgnoreCase(ServiceConst.SERVICE_GET_TEACHER_CONTACT_LIST)){
                    ...
                    // 添加显示第一个fragment
                    getSupportFragmentManager().beginTransaction().add(R.id.contact_fragment_container, familyFragment)
                            .add(R.id.contact_fragment_container, teacherFragment).hide(familyFragment).show(teacherFragment)
                            .commit();
                }
