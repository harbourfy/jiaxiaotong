    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.forget_password_activity_next_btn:
                if (userNameEt.getText().length() > 0 && passwordEt.getText().toString().length() > 5 && codeEt.getText().toString().length() > 3) {
                    startActivity(new Intent(ResetPasswordActivity.this, UserInfoActivity.class));
                    verifyAuthCode();
                }else if (TextUtils.isEmpty(userNameEt.getText().toString())){
                    ToastUtils.ToastMsg(activity,"手机号不能为空");
                    ...
                    ToastUtils.ToastMsg(activity,"验证码长度不能小于3位");
