...
import com.app.jiaxiaotong.fragment.ChatAllHistoryFragment;
import com.app.jiaxiaotong.fragment.ContactFragment;
...
import com.app.jiaxiaotong.im.db.InviteMessgeDao;
import com.app.jiaxiaotong.im.db.UserDao;
import com.app.jiaxiaotong.im.domain.InviteMessage;
...
//        MobclickAgent.updateOnlineConfig(this);
        ...
        fragments = new Fragment[] { chatHistoryFragment,applicationFragment, contactListFragment, mineFragment };
        // 添加显示第一个fragment
        ...
        init();
        //异步获取当前用户的昵称和头像
        ((MyHXSDKHelper) HXSDKHelper.getInstance()).getUserProfileManager().asyncGetCurrentUserInfo();
    }
    ...
    static void asyncFetchContactsFromServer(){
        HXSDKHelper.getInstance().asyncFetchContactsFromServer(new EMValueCallBack<List<String>>(){
            ...
            public void onSuccess(List<String> usernames) {
                ...
                ((MyHXSDKHelper)HXSDKHelper.getInstance()).getUserProfileManager().asyncFetchContactInfosFromServer(usernames,new EMValueCallBack<List<User>>() {

                    @Override
                    public void onSuccess(List<User> uList) {
                        ((MyHXSDKHelper)HXSDKHelper.getInstance()).updateContactList(uList);
                        ((MyHXSDKHelper)HXSDKHelper.getInstance()).getUserProfileManager().notifyContactInfosSyncListener(true);
                    }
    ...
    private void initView() {
        ...
        mTabs[1] = (Button) findViewById(R.id.btn_application_center);
    ...
    public void onTabClicked(View view) {
        switch (view.getId()) {
            case R.id.btn_conversation:
                index = 0;
                break;
            case R.id.btn_application_center:
                index = 1;
                break;
...
//					unreadAddressLable.setText(String.valueOf(count));
    ...
    public int getUnreadAddressCountTotal() {
        ...
        if (((MyHXSDKHelper)HXSDKHelper.getInstance()).getContactList().get(Constant.NEW_FRIENDS_USERNAME) != null)
            unreadAddressCountTotal = ((MyHXSDKHelper)HXSDKHelper.getInstance()).getContactList().get(Constant.NEW_FRIENDS_USERNAME)
                    .getUnreadMsgCount();
    ...
    public class MyContactListener implements EMContactListener {
        ...
        public void onContactAdded(List<String> usernameList) {
            // 保存增加的联系人
            Map<String, User> localUsers = ((MyHXSDKHelper)HXSDKHelper.getInstance()).getContactList();
    ...
    private void saveInviteMsg(InviteMessage msg) {
        ...
        // 未读数加1
        User user = ((MyHXSDKHelper)HXSDKHelper.getInstance()).getContactList().get(Constant.NEW_FRIENDS_USERNAME);
    ...
    private void showConflictDialog() {
        ...
        MyHXSDKHelper.getInstance().logout(false,null);
    ...
    private void registerInternalDebugReceiver() {
        internalDebugReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                MyHXSDKHelper.getInstance().logout(true,new EMCallBack() {
